...
* **Brazo extensor hacia spec-kit (opcional):** una extensión bundleada (`.specify/extensions/gomemory-context/`) conecta `get_context`/`mem context` con el flujo `/speckit-specify` (y opcionalmente `/speckit-plan`/`/speckit-clarify`) de [GitHub Spec Kit](https://github.com/github/spec-kit): cada especificación nueva se redacta con el historial del proyecto (features previas, decisiones) sin barrer `specs/` a mano, y sin mezclar esa historia con el grafo de código externo (secciones separadas y rotuladas). Se enciende/apaga desde la TUI o con `mem settings --speckit-context=true|false` (default activado), de forma independiente a la configuración de spec-kit. Ver [`docs/architecture.md`](docs/architecture.md).
