...
func New(lister ports.MemoryLister, session ports.SessionQuerier, root, project string) *Builder {
	return &Builder{Lister: lister, Session: session, Project: project, Root: root}
}
...
func (b *Builder) Build() (string, error) {
	...
	count := 0
	for _, m := range mems {
		if m.Type == domain.Architecture || m.Type == domain.Decision || m.Type == domain.Pattern || m.Type == domain.Bugfix {
			continue
		}
		if count >= 15 {
			break
