...
## Memoria Persistente (gomemory)

Este proyecto tiene integración MCP con `gomemory`. Tools disponibles: `save_memory`, `search_memories`, `list_memories`, `get_memory`, `forget_memory`, `judge_memories`, `start_session`, `end_session`, `get_context`. El servidor MCP se encarga del ciclo de vida de sesiones y contexto automáticamente. Usa `./mem` para acceder desde CLI si el MCP no está disponible.

Para más detalle del protocolo de memoria, ver `docs/MEMORY-PROTOCOL.md`.
