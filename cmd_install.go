...
func cmdInstall(args []string) {
	...
	if sameFile {
		...
		// Remove existing binary first to avoid "text file busy" on overwrite
	...
	dbPath := filepath.Join(target, store.MemDir, store.DbName)
	...
	for _, fname := range agentFiles {
		...
		if strings.Contains(content, integrationVersionMarker) {
			// Ya tiene el protocolo vigente
			continue
		}

		if idx := strings.Index(content, integrationMarker); idx != -1 {
			// Bloque antiguo: lo reemplazamos por la versión vigente
	...
	settings := store.ReadSettings(target)
	if settings.AutoApprove {
		store.ApplyAutoApprove(target, settings)
...
// integrationMarker marca el inicio del bloque inyectado por mem install.
// integrationVersionMarker identifica la versión del protocolo (para poder
// reemplazar bloques antiguos en upgrades sin duplicar contenido).
