...
func cmdCompare(args []string) {
	...
	mems, err := store.ListMemories(db, project, 200)
	if err != nil {
	...
	relType := types.ValidRelationType(*relation)
	...
	existing, _ := store.GetRelationByPair(db, project, id1, id2)
	if existing != nil {
		// Update existing
		if err := store.UpdateRelation(db, existing.ID, relType, *confidence, reasonText); err != nil {
		...
		id, err := store.InsertRelation(db, rel)
		if err != nil {
...
func cmdCompareList(args []string) {
	...
	project := filepath.Base(root)
	rels, err := store.ListRelations(db, project, *limit)
	if err != nil {
...
// Relation type descriptions for help
