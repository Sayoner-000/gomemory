...
func NewWithRepos(memRepo ports.MemoryRepository, sessRepo ports.SessionRepository, project string, port int) *Server {
	mux := http.NewServeMux()
	s := &Server{memRepo: memRepo, sessRepo: sessRepo, project: project, port: port, mux: mux}
	s.registerRoutes()
	return s
}
...
func (s *Server) handleSessionStart(w http.ResponseWriter, r *http.Request) {
	active, err := s.sessRepo.Active(s.project)
	if err != nil {
	...
	session, err := s.sessRepo.Start(s.project)
	if err != nil {
...
func (s *Server) handleSessionEnd(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.sessRepo.End(req.SessionID, req.Summary); err != nil {
...
func (s *Server) handleContext(w http.ResponseWriter, r *http.Request) {
	active, _ := s.sessRepo.Active(s.project)
	recentSessions, _ := s.sessRepo.Recent(s.project, 5)
	recentMemories, _ := s.memRepo.List(s.project, 10)
